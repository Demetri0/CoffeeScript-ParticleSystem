...
(function() {
  ...
  Vec2 = (function() {
    ...
    Vec2.getRandom = function(min, max) {
      return new Vec2(Math.random() * (max - min) + min);
    };
  ...
  World = (function() {
    ...
    World.prototype.update = function() {
      var ind, j, len, objects, ref, results;
      ref = this.objects;
      results = [];
      for (ind = j = 0, len = ref.length; j < len; ind = ++j) {
        objects = ref[ind];
        if (objects) {
          results.push(objects.update(ind));
        }
      }
      return results;
    };
  ...
  _Object = (function() {
    function _Object(config) {
      ...
      this.world = config = config.world;
    }
    ...
    _Object.prototype.notVisible = function(threshold) {
      return this.loc.y > this.world.convasHeight + threshold || this.loc.y < -threshold || this.loc.x > this.world.canvasWidth + threshold || this.loc.x < -threshold;
    };
